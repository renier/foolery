...
function buildShipmentPrompt(beatId: string, currentState: string): string {
  ...
  return [
    ...
    "   c. If there are merge conflicts, resolve them. If unresolvable, roll back to `ready_for_implementation`.",
    "   d. Switch to main and merge: `git checkout main && git merge <beat-branch> --no-ff`.",
    "   e. Push: `git push origin main`.",
    "3. If you are already on `main`, verify all implementation is committed and pushed.",
    "4. Verify any release or deployment prerequisites.",
    `5. Move to shipment review queue: \`${toShipmentReview}\`.`,
    "",
    "## Output",
    "- Implementation code merged to `main` and pushed.",
    "- Beat transitioned to `ready_for_shipment_review`.",
    "",
    "## Failure Modes",
    "- Merge conflicts: roll back to `ready_for_implementation` with a note describing the conflict.",
    "- No implementation found: roll back to `ready_for_implementation`.",
  ].join("\n");
}
...
function buildShipmentReviewPrompt(beatId: string, currentState: string): string {
  ...
  return [
    ...
    "2. Confirm shipment completeness and post-ship validation (tests pass, no regressions).",
    `3. If approved, close as shipped: \`${toShipped}\`.`,
    `4. If shipment fixes are needed, return to shipment queue: \`${toShipment}\`.`,
    `5. If deeper fixes are needed, return to implementation queue: \`${toImplementation}\`.`,
    "",
    "## Output",
